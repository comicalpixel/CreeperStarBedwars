...
public class PlayerShopDataConfig extends YamlConfiguration {
    ...
    public PlayerShopDataConfig(Plugin plugin) {
        this(plugin, "config.yml");
    }

    public PlayerShopDataConfig(Plugin plugin, File file) {
    ...
    public PlayerShopDataConfig(Plugin plugin, String filename) {
    ...
    public static void auto_reload() {
        new BukkitRunnable() {
            @Override
            public void run() {
                CreeperStarBedwars.getInstance().getShopDataConfig().reload();
            }
        }.runTaskTimer(CreeperStarBedwars.getInstance(), 0,20);
    }
